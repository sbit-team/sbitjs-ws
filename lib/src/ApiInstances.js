...
class ApisInstance {
    ...
    connect( cs, connectTimeout, enableCrypto = false ) {
        ...
        this.init_promise = this.ws_rpc.login(rpc_user, rpc_password).then(() => {
            ...
            this._db = new SBitApi(this.ws_rpc, "database");
            this._net = new SBitApi(this.ws_rpc, "network_broadcast");
            this._hist = new SBitApi(this.ws_rpc, "history");
            if (enableCrypto) this._crypt = new SBitApi(this.ws_rpc, "crypto");
